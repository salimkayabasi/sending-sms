...
var log = require('log-with')(module);
var phone = require('phone');
var _ = require('lodash');
var async = require('async');
var smsMaxLength = 160;
var providers = {};
var providerNames = _.filter(fs.readdirSync(path.resolve(__dirname + '/../providers')),
  function (file) {
...
exports.send = function (provider, to, text, cb) {
  ...
  async.concat(messages, providers[provider].send.bind(null, to),
    function (error, result) {
      if (error) {
        cb(error);
        return;
      }
      cb(null, _.map(result,
        function (response) {
          return {
            phone: to,
            text: text,
